...
void ProcessRpcRequest(InputMessageBase* msg_base) {
    ...
    // Schedule latency tracing logic
    if (FLAGS_enable_schedule_tracing && msg_base->msg_received_ns != 0) {
        ...
        if (count % FLAGS_schedule_tracing_sampling == 0) {
            if (FLAGS_v >= 1) {
                ...
                if (msg_base->bthread_scheduled_ns > msg_base->bthread_queued_ns) {
                    VLOG(1) << "  Stage[bthread_queued->bthread_scheduled]: "
                            << (msg_base->bthread_scheduled_ns - msg_base->bthread_queued_ns) << " ns";
                }
                if (msg_base->bthread_running_ns > msg_base->bthread_scheduled_ns) {
                    VLOG(1) << "  Stage[bthread_scheduled->bthread_running]: "
            ...
            // Attach trace info to response for client side collection
            char trace_buf[512];
