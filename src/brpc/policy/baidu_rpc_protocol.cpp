...
void SendRpcResponse(int64_t correlation_id, Controller* cntl,
    ...
    // Add schedule latency tracing info to response attachment before calculating size
    if (FLAGS_enable_schedule_tracing && msg_base->msg_received_ns != 0) {
        ...
        if (count % FLAGS_schedule_tracing_sampling == 0) {
            ...
            if (VLOG_IS_ON(1)) {
                ...
                if (msg_base->queue_msg_end_ns > msg_base->queue_msg_start_ns) {
                    VLOG(1) << "  Stage[queue_msg]: "
                            << (msg_base->queue_msg_end_ns - msg_base->queue_msg_start_ns) << " ns";
                }
                if (msg_base->bthread_queued_ns > msg_base->queue_msg_end_ns) {
                    VLOG(1) << "  Stage[queue_msg_end->bthread_queued]: "
                            << (msg_base->bthread_queued_ns - msg_base->queue_msg_end_ns) << " ns";
                }
                if (msg_base->bthread_signaled_ns > msg_base->bthread_queued_ns) {
                    VLOG(1) << "  Stage[bthread_queued->signaled]: "
                            << (msg_base->bthread_signaled_ns - msg_base->bthread_queued_ns) << " ns";
                }
                if (msg_base->bthread_stolen_ns > msg_base->bthread_signaled_ns) {
                    VLOG(1) << "  Stage[bthread_signaled->stolen]: "
                            << (msg_base->bthread_stolen_ns - msg_base->bthread_signaled_ns) << " ns";
                }
                if (msg_base->bthread_scheduled_ns > (msg_base->bthread_stolen_ns ? msg_base->bthread_stolen_ns : msg_base->bthread_signaled_ns)) {
                    VLOG(1) << "  Stage[bthread_" << (msg_base->bthread_stolen_ns ? "stolen" : "signaled") << "->scheduled]: "
                            << (msg_base->bthread_scheduled_ns - (msg_base->bthread_stolen_ns ? msg_base->bthread_stolen_ns : msg_base->bthread_signaled_ns)) << " ns";
                }
                if (msg_base->bthread_running_ns > msg_base->bthread_scheduled_ns) {
                    VLOG(1) << "  Stage[bthread_scheduled->running]: "
                            << (msg_base->bthread_running_ns - msg_base->bthread_scheduled_ns) << " ns";
                }
                if (msg_base->process_input_ns > msg_base->bthread_running_ns) {
                    VLOG(1) << "  Stage[bthread_running->process_input]: "
                            << (msg_base->process_input_ns - msg_base->bthread_running_ns) << " ns";
                }
                if (msg_base->process_rpc_ns > msg_base->process_input_ns) {
                    VLOG(1) << "  Stage[process_input->process_rpc]: "
                            << (msg_base->process_rpc_ns - msg_base->process_input_ns) << " ns";
                }
            }
