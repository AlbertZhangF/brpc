...
//
// Two cooperating subsystems for io_uring pre-registered I/O buffers.
//
// Background
// ----------
...
//
// Subsystem 1 – IouringMemPool  (global, one per process)
// --------------------------------------------------------
// Replaces butil::iobuf::blockmem_allocate so that EVERY IOBuf block comes
// from a pre-registered slab.  This means that IOBuf blocks on the write
// path can be handed directly to IORING_OP_WRITE_FIXED without any copy.
//
...
//
// Subsystem 2 – IouringReadSlotPool  (per-Poller-ring)
// ------------------------------------------------------
// Manages a set of fixed-size receive buffers ("read slots") for
// IORING_OP_READ_FIXED.  Each slot is one pre-registered buffer that is
// handed directly to a READ_FIXED SQE; the kernel DMA-writes received data
// into it without any per-op page pinning.
//
// Slots are always allocated from IouringMemPool (same pre-registered slab),
// so no additional io_uring_register_buffers call is needed per ring.
// Requires --iouring_register_buffers=true.
//
// Thread safety
// -------------
// IouringMemPool:     thread-safe (TLS fast-path + mutex for growth).
// IouringReadSlotPool: NOT thread-safe.  All Acquire/Release calls must
//                      happen on the Poller thread (enforced by design).
...
// IOBuf block pool (for WRITE_FIXED)
...
// Per-ring read slots (for READ_FIXED)
DECLARE_int32(iouring_read_slot_num);     // initial slots per ring (default 256)
DECLARE_int32(iouring_read_slot_max);     // max slots per ring (default 4096)

// ---------------------------------------------------------------------------
// IouringReadSlot – token held by an endpoint while a READ_FIXED is in flight
// ---------------------------------------------------------------------------
struct IouringReadSlot {
    void*  buf;        // pointer to the slot's memory
    int    buf_index;  // index in the ring's registered iovec table
    size_t size;       // slot size in bytes
    int    slot_idx;   // index into IouringReadSlotPool::entries_ (for O(1) Release)
};
...
//   IouringMemPool::Instance().Init(ring_registrar_callback);
//
//   // In CutFromIOBufList, for each IOBuf block:
//   int buf_index = IouringMemPool::Instance().GetBufIndex(block_ptr);
//   if (buf_index >= 0)
//       // submit IORING_OP_WRITE_FIXED(block_ptr, buf_index)
//   else
//       // fallback IORING_OP_WRITEV
// ---------------------------------------------------------------------------
...
public:
    ...
    // Called via blockmem_allocate hook.
    void* Allocate(size_t size);

    // Return a block to the pool.
    // Called via blockmem_deallocate hook.
    ...
    // miss the newest region and return -1 (caller falls back to WRITEV).
...
private:
    ...
    // Global free-list (protected by free_lock_).
    butil::Mutex              free_lock_;
    FreeNode*                 global_free_  = nullptr;
...
// ---------------------------------------------------------------------------
// IouringReadSlotPool  (per-Poller-ring, receive buffers for READ_FIXED)
//
// Owns a pool of pre-registered receive buffers.  Call Acquire() before
// submitting a READ_FIXED SQE to get a (buf, buf_index, size) triple;
// call Release() after the CQE is consumed and the data has been copied.
// ---------------------------------------------------------------------------
class IouringReadSlotPool {
public:
    IouringReadSlotPool() = default;
    ~IouringReadSlotPool() { Destroy(); }

    // Initialise the read-slot pool for |ring|.
    //
    // Requires IouringMemPool to be initialised (--iouring_register_buffers=true)
    // and |slot_buf_size| == MemPool block_size so that receive buffers are
    // allocated from the same pre-registered slab and the kernel can DMA-write
    // received data directly into them (IORING_OP_READ_FIXED) without per-op
    // page pinning.
    //
    // |initial_slots|  : number of receive buffers to pre-allocate.
    // |max_slot_count| : upper bound (controls the sparse table size).
    // |slot_buf_size|  : bytes per receive buffer (must equal
    //                    iouring_iobuf_block_size).
    bool Init(struct io_uring* ring,
              int initial_slots,
              int max_slot_count,
    ...
    // Acquire a slot (may grow the pool).
    bool Acquire(IouringReadSlot* out);

    // Return a slot to the pool.
    void Release(const IouringReadSlot& slot);

    bool   initialized()      const { return ring_ != nullptr; }
    size_t slot_buf_size()    const { return slot_buf_size_; }
    int    total_slot_count() const { return total_slot_count_; }
    int    free_count()       const { return static_cast<int>(free_slot_indices_.size()); }

private:
    bool GrowBy(int n);

    // One entry per allocated receive buffer.
    // Invariant: entries_[k].buf is the buffer for logical slot index k.
    // (GrowBy appends in order, so the vector index IS the slot index.)
    struct ReadSlotEntry {
        void* buf;  // pointer to the buffer (owned by IouringMemPool)
    };

    struct io_uring* ring_             = nullptr;
    size_t           slot_buf_size_    = 0;  // bytes per receive buffer
    int              max_slot_count_   = 0;  // hard upper bound on slots
    int              total_slot_count_ = 0;  // currently allocated slots
    std::vector<ReadSlotEntry> entries_;           // all allocated entries
    std::vector<int>           free_slot_indices_; // indices of free slots

    DISALLOW_COPY_AND_ASSIGN(IouringReadSlotPool);
};
...
// Returns -1 if ptr is not in a registered region (caller must fall back).
