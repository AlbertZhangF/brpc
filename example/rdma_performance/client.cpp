...
butil::atomic<int64_t> g_server_request_decompress_avg_ns;
butil::atomic<int64_t> g_server_request_decompress_p99_ns;
butil::atomic<int64_t> g_server_response_compress_avg_ns;
butil::atomic<int64_t> g_server_response_compress_p99_ns;
...
public:
    ...
    static void HandleResponse(RespClosure* closure) {
        ...
        g_server_request_decompress_avg_ns.store(
            closure->resp->server_request_decompress_avg_ns(),
            butil::memory_order_relaxed);
        g_server_request_decompress_p99_ns.store(
            closure->resp->server_request_decompress_p99_ns(),
            butil::memory_order_relaxed);
        g_server_response_compress_avg_ns.store(
            closure->resp->server_response_compress_avg_ns(),
            butil::memory_order_relaxed);
        g_server_response_compress_p99_ns.store(
            closure->resp->server_response_compress_p99_ns(),
            butil::memory_order_relaxed);
...
void Test(int thread_num, int attachment_size) {
    ...
    g_server_request_decompress_avg_ns.store(0, butil::memory_order_relaxed);
    g_server_request_decompress_p99_ns.store(0, butil::memory_order_relaxed);
    g_server_response_compress_avg_ns.store(0, butil::memory_order_relaxed);
    g_server_response_compress_p99_ns.store(0, butil::memory_order_relaxed);
    ...
    if (FLAGS_test_iterations == 0) {
        std::cout << "Avg-Latency: " << g_latency_recorder.latency(10)
            ...
            << ", ClientRequestCompressAvg: " << client_request_compress_avg_ns << "ns"
            << ", ClientRequestCompressP99: " << client_request_compress_p99_ns << "ns"
            << ", ServerRequestDecompressAvg: "
            << g_server_request_decompress_avg_ns.load(butil::memory_order_relaxed) << "ns"
            << ", ServerRequestDecompressP99: "
            << g_server_request_decompress_p99_ns.load(butil::memory_order_relaxed) << "ns"
            << ", ServerResponseCompressAvg: "
            << g_server_response_compress_avg_ns.load(butil::memory_order_relaxed) << "ns"
            << ", ServerResponseCompressP99: "
            << g_server_response_compress_p99_ns.load(butil::memory_order_relaxed) << "ns"
            << ", ClientResponseDecompressAvg: " << client_response_decompress_avg_ns << "ns"
            << ", ClientResponseDecompressP99: " << client_response_decompress_p99_ns << "ns"
            << std::endl;
    } else {
