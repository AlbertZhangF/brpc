...
butil::atomic<uint64_t> g_request_count(0);

namespace test {
class ExtremePerfTestServiceImpl : public PerfTestService {
public:
    ExtremePerfTestServiceImpl() {}
    ~ExtremePerfTestServiceImpl() {}
    ...
    void Test(google::protobuf::RpcController* cntl_base,
        ...
        g_request_count.fetch_add(1, butil::memory_order_relaxed);
...
int main(int argc, char* argv[]) {
    ...
    brpc::Server server;
    test::ExtremePerfTestServiceImpl service;
    ...
    std::cout << "Extreme Perf Server started on port " << FLAGS_port << std::endl;
    server.RunUntilAskedToQuit();

    uint64_t total = g_request_count.load(butil::memory_order_relaxed);
    std::cout << "Total requests served: " << total << std::endl;
