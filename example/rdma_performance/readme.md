...
本文档说明 `example/rdma_performance` 当前 client/server 的参数、可配置内容和推荐用法。当前示例既支持原始 protobuf attachment 压测，也支持 HTTP/H2 raw JSON body 压测；通过 `--transport=tcp|rdma|iouring` 选择底层传输。
...
| `--transport` | `rdma` | 传输模式：`tcp`、`rdma` 或 `iouring`。显式设置后优先级高于 `--use_rdma`。 |
| `--use_rdma` | `true` | 兼容旧脚本的参数，仅当未显式设置 `--transport` 时生效：`true` 等价于 `--transport=rdma`，`false` 等价于 `--transport=tcp`。 |
...
| `--transport` | `rdma` | 传输模式：`tcp`、`rdma` 或 `iouring`。显式设置后优先级高于 `--use_rdma`。client 侧的 `iouring` 与 `example/iouring_echo_c++` 保持一致，使用普通 TCP Channel 连接已启用 io_uring 的 server。 |
| `--use_rdma` | `true` | 兼容旧脚本的参数，仅当未显式设置 `--transport` 时生效：`true` 等价于 `--transport=rdma`，`false` 等价于 `--transport=tcp`。 |
