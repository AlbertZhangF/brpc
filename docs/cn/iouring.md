...
> **注意**：CMake 变量名为 `IOURING_INCLUDE_PATH` 和 `IOURING_LIB`，不是 `LIBURING_INCLUDE_PATH` / `LIBURING_LIBRARY`，拼写错误会触发 CMake Warning 且实际不生效。
...
| 零拷贝接收 | ✅（`READ_FIXED`，预注册 slot） | ✅（内存注册 + DMA） |
