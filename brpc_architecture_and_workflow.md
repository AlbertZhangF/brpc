# brpc框架架构与rdma\_performance工作流分析文档
...
- **核心功能**：
  - 协程创建与管理：bthread\_start\_background/urgent等接口
  - 工作窃取调度：每个Worker线程有独立运行队列，空闲时偷取其他队列任务
  - 协程同步原语：bthread\_mutex\_t、bthread\_cond\_t、bthread\_countdown\_event等
...
- **核心子模块**：
  ...
  - **协议层**：支持baidu\_std、HTTP、gRPC、Thrift、Redis等多种协议
...
| <br />   | `bthread_start_urgent(bthread_t* tid, const bthread_attr_t* attr, void *(*fn)(void*), void* arg)`     | 创建高优先级bthread，加入当前Worker的紧急队列，优先调度   | 紧急任务场景，需要立即执行的高优先级请求、超时回调等              |
| <br />   | `bthread_run(bthread_t* tid, const bthread_attr_t* attr, void *(*fn)(void*), void* arg)`              | 同步运行bthread，当前线程阻塞等待任务执行完成           | 需要同步等待结果的场景，简化异步逻辑                      |
| **调度控制** | `bthread_yield()`                                                                                     | 当前bthread主动让出CPU，回到运行队列尾部重新调度        | 计算密集型任务主动让渡资源，避免长时间独占Worker线程           |
| <br />   | `bthread_usleep(uint64_t us)`                                                                         | bthread休眠指定微秒，休眠期间会让出CPU，不阻塞Worker线程 | 定时等待场景，替代系统`usleep()`，避免阻塞Worker        |
| <br />   | `bthread_setconcurrency(int concurrency)`                                                             | 设置bthread Worker线程数量，默认等于CPU核数       | 性能调优，根据业务场景调整并发度                        |
| <br />   | `bthread_getconcurrency()`                                                                            | 获取当前Worker线程数量                       | 监控和调优                                   |
| **同步原语** | `bthread_mutex_init/destroy/lock/trylock/unlock`                                                      | bthread互斥锁，阻塞时会让出CPU给其他协程            | 协程间数据同步，比pthread\_mutex更轻量，不会阻塞Worker线程 |
| <br />   | `bthread_cond_init/destroy/wait/signal/broadcast`                                                     | bthread条件变量，与bthread\_mutex配合使用      | 协程间事件通知、生产者消费者模型                        |
| <br />   | `bthread_countdown_event_init/destroy/wait/signal`                                                    | 倒计时事件，等待N个任务完成                       | 批量任务同步，等待多个异步任务全部完成                     |
| <br />   | `bthread_rwlock_init/destroy/rdlock/wrlock/unlock`                                                    | 读写锁，支持多个读者、单个写者                      | 读多写少的同步场景，提高并发度                         |
| <br />   | `bthread_barrier_init/destroy/wait`                                                                   | 屏障，多个协程到达屏障点后同时继续                    | 并行计算场景，所有计算任务准备完成后同时启动                  |
| **本地存储** | `bthread_key_create(bthread_key_t* key, void (*destructor)(void*))`                                   | 创建bthread局部存储key，支持析构函数              | 协程级全局变量存储，每个bthread有独立的副本               |
| <br />   | `bthread_getspecific(bthread_key_t key)`                                                              | 获取当前bthread的局部存储数据                   | 读取协程私有数据，如请求上下文、 tracing信息等             |
| <br />   | `bthread_setspecific(bthread_key_t key, const void* value)`                                           | 设置当前bthread的局部存储数据                   | 存储协程私有数据                                |
| **调度统计** | `bthread_sched_latency_ns()`                                                                          | 获取当前bthread从创建到开始执行的总调度耗时            | 性能分析，统计整个调度过程的延迟                        |
| <br />   | `bthread_queue_latency_ns()`                                                                          | 获取当前bthread在运行队列中的等待时间               | 分析队列阻塞情况，定位调度瓶颈                         |
| <br />   | `bthread_switch_latency_ns()`                                                                         | 获取当前bthread上下文切换耗时                   | 分析调度开销                                  |
| <br />   | `bthread_enqueue_prepare_latency_ns()`                                                                | 获取任务创建到入队的准备耗时                       | 分析任务初始化开销                               |
...
## 2. rdma\_performance示例端到端工作流

### 2.1 示例概述

rdma\_performance是brpc提供的RDMA性能测试示例，用于测试RDMA传输模式下brpc的极限性能，包含客户端和服务端两个部分。
...
| --- | ---------------- | -------- | ------------------------- |
| 客户端 | 初始化阶段，每个并发线程创建一个 | 整个压测过程   | 等于并发数（FLAGS\_thread\_num） |
...
客户端在初始化阶段通过`bthread_start_background()`创建长期运行的发送线程，每个并发线程对应一个bthread：

```cpp
// 代码位置: example/rdma_performance/client.cpp:355
...
发送线程的入口函数`PerformanceTest::RunTest()`实现了循环发送机制：

```cpp
// 代码位置: example/rdma_performance/client.cpp:188-194
...
客户端发送线程不是通过传统的while循环来持续发送，而是采用**回调驱动**的模式：

1. **初始发送**：RunTest函数初始发送`FLAGS_queue_depth`个请求
2. **异步等待**：每个请求都是异步RPC调用，发送后立即返回，不阻塞bthread
3. **回调触发**：响应到达后触发`HandleResponse`回调
4. **循环发送**：在回调中继续调用`SendRequest()`发送下一个请求

```cpp
// 代码位置: example/rdma_performance/client.cpp:248
// HandleResponse回调函数中
closure->test->SendRequest(); // 形成循环：发送 -> 等待响应 -> 回调中继续发送
```
...
```cpp
// 代码位置: example/rdma_performance/client.cpp:242-246
// 在HandleResponse回调中检查结束条件
if (closure->cntl->Failed()) {
    closure->test->_stop = true;  // RPC调用失败
    return;
}
if (closure->test->_iterations == 0 && FLAGS_test_iterations > 0) {
    closure->test->_stop = true;  // 达到迭代次数
    return;
}
if (now - closure->test->_start_time > FLAGS_test_seconds * 1000000u) {
    closure->test->_stop = true;  // 达到测试时间
    return;
}
```

当`_stop`被设置为true后，回调中不再调用`SendRequest()`，所有pending的请求处理完成后，RunTest函数返回，bthread自然结束。

**3. 为什么客户端bthread不需要频繁销毁**

客户端采用长期运行bthread的设计，原因如下：

| 设计考虑     | 说明                                         |
| -------- | ------------------------------------------ |
| **性能优化** | 避免了频繁创建销毁bthread的开销，一个bthread可以处理成千上万个请求   |
| **异步模式** | RPC调用是异步的，发送请求后立即返回，不阻塞bthread             |
| **回调驱动** | 响应到达后触发回调，回调中继续发送下一个请求，形成高效循环              |
| **资源效率** | 不需要为每个请求创建新的bthread，大大减少了TaskMeta和栈内存的分配释放 |

#### 2.2.3 服务端bthread销毁机制详解

**1. 服务端bthread的创建**

服务端每收到一个请求，都会创建一个新的bthread来处理：

```cpp
// 代码位置: src/brpc/input_messenger.cpp:181
...
服务端bthread在请求处理完成后立即销毁，销毁流程在`TaskGroup::task_runner()`函数中：

```cpp
// 代码位置: src/bthread/task_group.cpp:394-536
void TaskGroup::task_runner(intptr_t skip_remained) {
    ...
    do {
        ...
        void* thread_return;
        try {
            thread_return = m->fn(m->arg);  // 执行ProcessInputMessage
...
// 代码位置: src/bthread/task_group.cpp:527-536
...
服务端bthread销毁时，资源不是真正释放，而是归还到对象池复用：

| 资源类型           | 管理方式                               | 复用机制                                                          |
| -------------- | ---------------------------------- | ------------------------------------------------------------- |
| **TaskMeta对象** | 全局对象池`butil::ObjectPool<TaskMeta>` | 通过`return_resource()`归还，下次创建bthread时直接从池中获取                   |
| **栈内存（小栈）**    | 小栈内存池，默认32KB                       | 通过`StackFactory<SmallStackClass>::return_stack()`归还，复用已分配的栈内存 |
| **栈内存（普通栈）**   | 普通栈内存池，默认1MB                       | 通过`StackFactory<NormalStackClass>::return_stack()`归还          |
| **栈内存（大栈）**    | 大栈内存池，默认8MB                        | 通过`StackFactory<LargeStackClass>::return_stack()`归还           |

**资源复用的优势**：

- **避免频繁内存分配**：TaskMeta和栈内存都是预分配的对象池，避免了频繁的malloc/free
- **提高性能**：从对象池获取资源比从系统分配快得多
- **减少内存碎片**：对象池管理避免了内存碎片化

**4. 为什么服务端需要为每个请求创建独立bthread**

服务端采用"一请求一bthread"的设计，原因如下：

| 设计考虑     | 说明                                      |
| -------- | --------------------------------------- |
| **请求隔离** | 每个请求在独立的bthread中处理，互不影响，一个请求的异常不会影响其他请求 |
| **并发控制** | 通过bthread数量控制并发度，避免资源耗尽                 |
| **公平调度** | bthread调度器可以公平地调度所有请求，避免某个请求长时间占用CPU    |
| **简化编程** | 每个请求的处理流程独立，不需要考虑请求间的交互和同步              |

#### 2.2.4 客户端与服务端bthread生命周期对比总结

| 对比维度     | 客户端发送线程                                                                                                      | 服务端处理线程                                                                                                           |
| -------- | ------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| **创建时机** | 初始化阶段，每个并发线程创建一个                                                                                             | 每收到一个请求创建一个                                                                                                       |
| **创建位置** | [client.cpp:355](file:///home/zfz/code/brpc/apache-brpc-1.15.0-src/example/rdma_performance/client.cpp#L355) | [input\_messenger.cpp:181](file:///home/zfz/code/brpc/apache-brpc-1.15.0-src/src/brpc/input_messenger.cpp#L181)   |
| **任务函数** | `PerformanceTest::RunTest()`                                                                                 | `ProcessInputMessage()`                                                                                           |
| **生命周期** | 整个压测过程（秒级到分钟级）                                                                                               | 单个请求处理周期（毫秒级）                                                                                                     |
| **工作模式** | 回调驱动循环发送，异步等待响应                                                                                              | 处理单个请求后立即返回                                                                                                       |
| **结束条件** | 测试时间到/迭代次数到/RPC失败                                                                                            | 请求处理完成，函数返回                                                                                                       |
| **销毁位置** | RunTest函数返回后自然销毁                                                                                             | [task\_group.cpp:527-536](file:///home/zfz/code/brpc/apache-brpc-1.15.0-src/src/bthread/task_group.cpp#L527-L536) |
| **资源复用** | 不需要频繁创建销毁，效率高                                                                                                | TaskMeta和栈内存被复用                                                                                                   |
| **设计优势** | 避免频繁创建销毁开销，一个bthread处理多个请求                                                                                   | 请求隔离，并发控制，公平调度                                                                                                    |

**核心差异**：客户端发送线程是**长期运行的任务**，采用回调驱动模式复用bthread；服务端处理线程是**短期任务**，采用"一请求一bthread"模式保证隔离性和公平性。
...
| brpc协议    | `src/brpc/policy/baidu_rpc_protocol.cpp` | baidu\_std协议实现 |

### 3.2 rdma\_performance示例代码位置
...
- **队列等待时间**：dequeue\_ns - enqueue\_ns，占调度总耗时的99%以上，是调度瓶颈的主要来源
...
| 对象                | 数量关系                                | 核心职责                        |
| ----------------- | ----------------------------------- | --------------------------- |
| Worker线程          | 等于bthread\_concurrency配置值，默认等于CPU核数 | 内核级线程，执行bthread任务           |
...
4. **唤醒机制**：使用pipe实现优雅退出，向`_wakeup_fds[1]`写入数据触发epoll\_wait返回
